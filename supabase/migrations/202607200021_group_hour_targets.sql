alter table public.agent_groups
  add column weekly_target_minutes integer
    check (weekly_target_minutes between 0 and 10080),
  add column monthly_target_minutes integer
    check (monthly_target_minutes between 0 and 44640);
...
begin
  ...
  join public.agent_group_memberships membership
    on membership.group_id = agent_group.id
  ...
  if target_group.weekly_target_minutes is not null then
    weekly_target_minutes := target_group.weekly_target_minutes;
    target_source := 'group';
  end if;

  if target_group.monthly_target_minutes is not null then
